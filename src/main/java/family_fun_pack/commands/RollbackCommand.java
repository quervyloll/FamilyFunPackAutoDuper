...
public class RollbackCommand extends Command implements PacketListener {
  ...
  public String execute(String[] args) {
    ...
    if(teleport_id == -1) { // Init a rollback
      ...
      Packet<?>[] to_send = new Packet<?>[3];
        ...
        if(mode == Mode.SIMPLE) {
          to_send[0] = new CPacketConfirmTeleport(teleport_id); // Set position server-side
          ...
          to_send[0] = new CPacketConfirmTeleport(teleport_id);
      ...
      return "Rollback to idkkk position.";
    }
  ...
  public Packet<?> packetReceived(EnumPacketDirection direction, int id, Packet<?> packet, ByteBuf in) {
    ...
    if(id == 47) { // SPacketPlayerPosLook, rollback initialisation
      ...
      if(x != this.position.x || y != this.position.y || z != this.position.z) {
        ...
        Vec3d loc = this.outputedPos();
        FamilyFunPack.printMessage("Rollback set to current position.");
      }

      teleport_id = position.getTeleportId();
